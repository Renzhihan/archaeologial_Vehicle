...
void
OdomCallback(const nav_msgs::Odometry odompose){

ros::NodeHandle nh;
ros::Publisher pose_pub_ = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("amcl_pose", 2, true);


geometry_msgs::PoseWithCovarianceStamped lastpose;
lastpose.pose.pose.position.x =odompose.pose.pose.position.x ;
lastpose.pose.pose.position.y =odompose.pose.pose.position.y ;
lastpose.pose.pose.position.z = 0.0;
lastpose.header.stamp = ros::Time::now();
//tf2::getYaw(lastpose.pose.pose.orientation) = tf2::getYaw(odompose.pose.pose.orientation);

// geometry_msgs::TransformStamped map_trans;
// map_trans.header.frame_id = "map";
// map_trans.header.stamp = ros::Time::now();
// map_trans.child_frame_id = "odom";
// map_trans.transform.translation.x;
...
main(int argc, char** argv)
{
  ...
  ros::Publisher pose_pub_ = nh_.advertise<geometry_msgs::PoseWithCovarianceStamped>("amcl_pose", 2, true);
  odom_sub_ = nh_.subscribe<nav_msgs::Odometry>("/odom",1,OdomCallback);
*/
/*
